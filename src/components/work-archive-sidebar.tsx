...
interface WorkArchiveSidebarProps {
  entries: WorkArchiveEntry[];
  currentSlug?: string;
  basePath?: string;
}

export function WorkArchiveSidebar({
  entries,
  currentSlug,
  ...
  nonFavorites.forEach((entry) => {
    const label = entry.groupLabel || entry.date;
    if (!groups.has(label)) {
      groups.set(label, []);
    }
      ...
      <div className="space-y-2">
        {Array.from(groups.entries())
          .sort(([a], [b]) => {
            ...
            const dateA = groups.get(a)?.[0]?.date || a;
            const dateB = groups.get(b)?.[0]?.date || b;
